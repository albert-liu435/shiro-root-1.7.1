...
public abstract class AccessControlFilter extends PathMatchingFilter {
     ...
     * 便利方法仅委托给 WebUtils.saveRequest请求（request）}保存请求状态以供以后重用。这主要用于在发出重定向以将用户返回到其最初请求的url/资源时保留用户请求状态。
    ...
    /**
     * 对于只获取{@link#getLoginUrl（）getLoginUrl}并将请求重定向到该url的子类的方便方法。
