...
public abstract class AbstractShiroFilter extends OncePerRequestFilter {
    ...
    /**
     * 检查是否已配置将筛选器配置为使用静态内存的init参数，如果已配置，则使用配置的值设置{@link#setStaticSecurityManagerEnabled（boolean）}属性。
    ...
    /**
     * 准备将传递给{@code FilterChain}的{@code ServletRequest}实例，以进行请求处理。
