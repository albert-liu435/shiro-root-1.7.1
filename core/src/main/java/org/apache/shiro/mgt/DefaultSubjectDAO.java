...
/**
 * SubjectDAO的默认实现类
 * 默认{@code SubjectDAO}实现，默认情况下在主题的会话中存储主题状态（但可以禁用此功能-请参阅下文）。
 * <p>
...
public class DefaultSubjectDAO implements SubjectDAO {
    ...
    /**
     * 确定是否使用主题的会话来持久化主题状态。此实现仅委托给内部{@link SessionStorageEvaluator}（默认情况下是{@code DefaultSessionStorageEvaluator}）。
    ...
    /**
     * 设置SessionStorageEvaluator用于决定Subject的状态是否保存在Session中
