 ...
 * <p>
 * <p>
 * 主体，代表了当前“用户”，这个用户不一定是一个具体的人，与当前应用交互的任何东西都是Subject，如网络爬虫，机器人等；即一个抽象概念；所有Subject都绑定到SecurityManager，
 ...
 * <p>
 * <p>
 * 主体，应用代码直接交互的对象是 Subject，也就是说 Shiro 的对外 API 核心就是 Subject , 代表了当前“用户”，这个用户不一定是一个具体的人，与当前应用交互的任何东西都是Subject，如网络爬虫，机器人等；即一个抽象概念；
...
public interface Subject {
     ...
     * <p>
     * 如果允许此主题执行操作或访问由指定权限字符串汇总的资源，则返回{@code true}。
    ...
    /**
     * Returns {@code true} if this Subject/user proved their identity <em>during their current session</em> by providing valid credentials matching those known to the system,
     * 返回{@code true}如果此主题/用户在其当前会话期间通过提供与系统已知凭据匹配的有效凭据来证明其身份，
